...
static ble_hids_t * mp_hids = NULL;                         /**< Pointer to structure used to identify the HID service. */
 ...
 */
static void bas_init(ble_bas_t * p_bas)
{
    uint32_t       err_code;
    ...
    err_code = ble_bas_init(p_bas, &bas_init_obj);
    APP_ERROR_CHECK(err_code);
}
...
static void hids_init(ble_hids_t * p_hids)
{
    ...
    // store a pointer to the HID Service structure
    mp_hids = p_hids;
                                            ...
                                            uint16_t   * p_actual_len)
{
    ...
    do
    {
        ...
        // TODO: button press as SHIFT key ?
...
uint32_t send_key_scan_code(uint8_t   key_scan_code)
{
	...
	if (key_scan_code != 0) // Not a key release
	{
		// Copy the scan code.
		memcpy(data + SCAN_CODE_POS, &key_scan_code, 1);
	}

	if (!m_in_boot_mode)
	{
		err_code = ble_hids_inp_rep_send(mp_hids,
										 INPUT_REPORT_KEYS_INDEX,
										 INPUT_REPORT_KEYS_MAX_LEN,
										 data);
	}
	...
	{
		err_code = ble_hids_boot_kb_inp_rep_send(mp_hids,
												 INPUT_REPORT_KEYS_MAX_LEN,
												 data);
	}
...
void keys_send(uint8_t key_pattern_len, uint8_t * p_key_pattern)
{
    ...
    err_code = send_key_scan_press_release(mp_hids,
                                           p_key_pattern,
                                           key_pattern_len,
                                           0,
                                           &actual_len);
    ...
    if ((err_code == BLE_ERROR_NO_TX_PACKETS) && (actual_len <= key_pattern_len))
    {
        ...
        UNUSED_VARIABLE(buffer_enqueue(mp_hids, p_key_pattern, key_pattern_len, actual_len));
    }
...
static void on_hid_rep_char_write(ble_hids_evt_t * p_evt)
{
    ...
    if (p_evt->params.char_write.char_id.rep_type == BLE_HIDS_REP_TYPE_OUTPUT)
    {
        ...
        if (report_index == OUTPUT_REPORT_INDEX)
        {
            ...
            err_code = ble_hids_outp_rep_get(mp_hids,
                                             report_index,
